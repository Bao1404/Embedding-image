...
def _scrape_one_expansion(store_id, info, url, dry_run, progress_lock):
    from playwright.sync_api import sync_playwright
    db = get_sync_db()
    ...
    with sync_playwright() as p:
        ...
        logging.info(f"  [{store_id}] Found {len(unique_entries)} unique cards")
        ...
        for i in range(len(unique_entries)):
            ...
            if (i+1) % 10 == 0:
                logging.info(f"  [{store_id}] [{i+1}/{len(unique_entries)}] {entry['name_number']}")
            ...
            try:
                ...
                logging.error(f"  [{store_id}] Error scraping card {entry['href']}: {e}")
                errors_count += 1
...
def cmd_rescrape(dry_run=False, limit=None, workers=3):
    logging.info("=" * 60)
    logging.info(f"RESCRAPE MODE — One-time full overwrite (Workers: {workers})")
    logging.info("=" * 60)
    ...
    if limit and limit > 0:
        ...
        if len(remaining) > limit:
            logging.info(f"--limit {limit}: chi xu ly {limit} expansion(s) chua xong")
    ...
    remaining_stores = [s for s in all_store_ids if s not in completed_stores]
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for store_id in remaining_stores:
            ...
            logging.info(f"Submitting: {store_id} ({url})")
            f = pool.submit(_scrape_one_expansion, store_id, info, url, dry_run, progress_lock)
            futures.append(f)
...
def main():
    ...
    parser.add_argument("--workers", type=int, default=3, help="Number of parallel workers for rescrape")
    ...
    if args.update:
        ...
        cmd_rescrape(dry_run=args.dry_run, limit=args.limit, workers=args.workers)
